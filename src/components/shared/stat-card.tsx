...
interface StatCardProps {
  ...
  value: string | number;
  unit?: string;
  /** Month-over-month delta. Pass `null` (or `undefined`) to HIDE the trend
   *  indicator — used by Fase 4.10 to avoid showing misleading static
   *  numbers when no comparable historical data exists. */
  trend?: number | null;
  trendLabel?: string;
  iconColor?: string;
  iconBg?: string;
...
export function StatCard({
  ...
  // Fase 4.10: `trend === null` means "no comparable data" → hide the badge.
      ...
      <div className="relative flex items-start justify-between">
        <div className="min-w-0 flex-1">
          ...
          {!showTrend && trendLabel && (
            // When no trend value is available, show only the qualitative
            // trend label (e.g. "perlu pantauan") without a misleading number.
            <div className="mt-2">
