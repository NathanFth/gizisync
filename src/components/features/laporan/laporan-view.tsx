...
export function LaporanView() {
  // Fase 3.8: Laporan view is filtered to 'Pengukuran Balita' only — the
  // 'Pemeriksaan Ibu Hamil' module is disabled in the demo build, so we no
  // longer pull ibuHamilList and no longer offer the Ibu Hamil filter option.
  const { balitaList, pengukuranList } = useStore();
  const [bulan, setBulan] = useState(new Date().getMonth());
  const [tahun, setTahun] = useState(new Date().getFullYear());
  // filterJenis kept for forward-compat but always 'Pengukuran Balita' now.
  const [filterJenis] = useState<string>('Pengukuran Balita');

  // Generate report records from actual pengukuran data (Balita only)
  const records = useMemo(() => {
    ...
    return balitaRecords as NonNullable<(typeof balitaRecords)[number]>[];
  }, [balitaList, pengukuranList]);

  const filteredRecords = useMemo(() => {
    return records.filter((r) => {
      if (r.bulan !== bulan || r.tahun !== tahun) return false;
      // Always 'Pengukuran Balita' — filter is a no-op but kept for clarity.
  ...
  const stats = useMemo(() => ({
    ...
    // Ibu Hamil KEK is always 0 now that the module is disabled in the demo.
    ibuHamilKEK: 0,
  }), [filteredRecords]);
      ...
      const html = `
        ...
        <body>
          ...
          <div class="summary">
            ...
            <div class="summary-card"><div class="label">Kasus Wasting Baru</div><div class="value">${filteredRecords.filter((r) => r.keterangan.includes('Wasting')).length}</div></div>
          </div>
          <table>
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="grid grid-cols-2 gap-3 sm:flex sm:items-end">
            ...
            {/* Fase 3.8: Jenis filter removed — only 'Pengukuran Balita' is in scope for the demo. */}
          </div>
      ...
      {/* Summary cards (Fase 3.8: Ibu Hamil KEK card replaced with Wasting) */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <StatCard icon={AlertTriangle} label="Kasus Wasting Baru" value={filteredRecords.filter((r) => r.keterangan.includes('Wasting')).length} iconColor="text-rose-600" iconBg="bg-rose-50 dark:bg-rose-500/10" />
      </div>
