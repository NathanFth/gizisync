...
export function IbuHamilDetailView() {
  ...
  if (!ibu) {
    return (
      <EmptyState
        ...
        action={
          <Button onClick={() => setView("ibu-hamil")}>
            Kembali ke Daftar
          </Button>
        }
      />
  ...
  const now = new Date();
  const daysToHtp = Math.ceil(
    (htpDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
  );
  const isHtpPassed = daysToHtp < 0;

  // Calculate trimester
  const trimester =
    ibu.usiaKehamilanMinggu < 13 ? 1 : ibu.usiaKehamilanMinggu < 27 ? 2 : 3;
  ...
  const ancCompleted = ancTypes.filter((t) =>
    ancVisits.some((v) => v.jenis === t),
  );
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {ibu.usiaKehamilanMinggu} minggu · Trimester {trimester} · NIK:{" "}
              {ibu.nik}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        ...
        <div className="space-y-5">
          <Card className="p-5">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5">
                  <IbuHamilStatusBadge status={ibu.status} />
                </div>
              </div>
            </div>
            <div className="mt-4 space-y-2.5 text-sm">
              <InfoRow
                ...
                value={`${ibu.usiaKehamilanMinggu} minggu (Trimester ${trimester})`}
              />
              <InfoRow
                icon={Calendar}
                label="HTP"
                value={formatTanggalPanjangID(ibu.htp)}
              />
              <InfoRow
                icon={Weight}
                label="Berat Badan"
                value={ibu.beratBadanKg ? `${ibu.beratBadanKg} kg` : "-"}
              />
              <InfoRow
                icon={Activity}
                label="Tekanan Darah"
                value={ibu.tekananDarah || "-"}
              />
              <InfoRow
                icon={Ruler}
                label="LILA Terakhir"
                value={`${ibu.lilaTerakCm} cm`}
              />
              {ibu.lilaTerakCm < AMBANG_LILA_KEK && (
                <div className="flex items-start gap-2 rounded-lg bg-amber-50 dark:bg-amber-500/10 p-2.5 text-amber-800 dark:text-amber-300">
                  ...
                  <p className="text-xs font-medium">
                    Risiko KEK (LILA &lt; {AMBANG_LILA_KEK} cm)
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="text-center">
              {isHtpPassed ? (
                ...
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    hari lagi sampai persalinan
                  </p>
                  <p className="mt-2 text-xs text-muted-foreground">
                    {formatTanggalID(ibu.htp)}
                  </p>
                </>
              )}
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (ibu.usiaKehamilanMinggu / 40) * 100)}%`,
                  }}
                />
              </div>
            </div>
          ...
          <Card className="p-5">
            <h3 className="mb-3 flex items-center gap-2 text-sm font-semibold text-foreground">
              <Stethoscope className="h-4 w-4 text-emerald-600" /> Kelengkapan
              ANC
            </h3>
            <div className="grid grid-cols-4 gap-2">
              {ancTypes.map((t) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-sm font-bold ${done ? "text-emerald-700 dark:text-emerald-400" : "text-muted-foreground"}`}
                    >
                      {t}
                    </span>
                    <span
                      className={`text-[10px] ${done ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}
                    >
                      {done ? "✓ Selesai" : "Belum"}
                    </span>
                  </div>
        ...
        <div className="space-y-5 lg:col-span-2">
          ...
          <Card className="p-5">
            <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-foreground">
              <TrendingUp className="h-4 w-4 text-rose-600" /> Progres Berat
              Badan & LILA
            </h3>
          ...
          <Card className="overflow-hidden">
            <div className="flex items-center justify-between border-b border-border p-4">
              <h3 className="flex items-center gap-2 text-sm font-semibold text-foreground">
                <Stethoscope className="h-4 w-4 text-rose-600" /> Riwayat
                Kunjungan ANC
              </h3>
              <Button
                size="sm"
                    ...
                    {ancVisits.map((v) => (
                      ...
                      >
                        <td className="px-4 py-3 text-muted-foreground">
                          {formatTanggalID(v.tanggalKunjungan)}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`font-semibold ${v.lilaCm < AMBANG_LILA_KEK ? "text-red-600 dark:text-red-400" : "text-foreground"}`}
                          >
                            {v.lilaCm}
                          </span>
                        </td>
            ...
            {ancVisits.length > 0 && (
              <div className="border-t border-border bg-muted/20 p-3 text-xs text-muted-foreground">
                <strong>Keterangan:</strong> TF = Tinggi Fundus (cm), DJJ =
                Denyut Jantung Janin (bpm), TD = Tekanan Darah
              </div>
            )}
          </Card>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select
                value={jenis}
                onValueChange={(v) => setJenis(v as JenisPemeriksaanANC)}
              >
                <SelectTrigger id="anc-jenis">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                max={45}
                value={usiaMinggu}
                onChange={(e) => setUsiaMinggu(e.target.value)}
              />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                step="0.1"
                value={fundus}
                onChange={(e) => setFundus(e.target.value)}
                placeholder="20"
              />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              placeholder="Misal: kehamilan normal, diberikan PMT, dll"
            />
          </div>
