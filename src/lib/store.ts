...
export function useDashboardStats() {
  ...
  return useMemo(() => {
    ...
    // Fase 4.10: Compute REAL month-over-month deltas instead of returning
    // the previously-hardcoded misleading static values (3 / 1 / 2.4).
    //
    //   • perubahanBalitaBulanIni   → # balita added this calendar month.
    //   • perubahanIbuHamilBulanIni → # ibu hamil added this calendar month
    //                                 (kept for type compat; module is disabled
    //                                 so this is effectively 0 in the demo).
    //   • perubahanPersentaseGiziBaik → difference between this month's
    //                                   % gizi baik and last month's, computed
    //                                   from the pengukuran records that fall
    ...
    // New balita this month: we don't track createdAt, so we approximate by
    // counting balita whose FIRST pengukuran falls in this month. This is a
    // reasonable proxy for "newly enrolled this month" in a demo build.
    const newBalitaThisMonth = balitaList.filter((b) => {
      const first = pengukuranList
        .filter((p) => p.balitaId === b.id)
        .sort((a, c) => new Date(a.tanggalPengukuran).getTime() - new Date(c.tanggalPengukuran).getTime())[0];
      return first ? isSameMonthYear(first.tanggalPengukuran, thisMonth, thisYear) : false;
    }).length;

    // Gizi-baik % for this month and last month, computed from pengukuran
    // records in each respective month. Only the LATEST pengukuran per balita
    // in each window counts.
    const giziBaikPercentForMonth = (m: number, y: number) => {
      const monthlyLatest = balitaList.map((b) => {
        const list = pengukuranList
          .filter((p) => p.balitaId === b.id && isSameMonthYear(p.tanggalPengukuran, m, y))
          .sort((a, c) => new Date(c.tanggalPengukuran).getTime() - new Date(a.tanggalPengukuran).getTime());
        return list[0] ?? null;
      }).filter(Boolean);
      if (monthlyLatest.length === 0) return null;
      const baik = monthlyLatest.filter((p) => p!.statusGizi === 'Normal').length;
    ...
    // If either month has no data, we cannot compute a meaningful delta —
    // return null so the UI can hide the trend indicator instead of
    // displaying a misleading static number.
    const perubahanPersentaseGiziBaik: number | null =
      thisMonthPct !== null && lastMonthPct !== null ? Number((thisMonthPct - lastMonthPct).toFixed(1)) : null;

    return {
      totalBalita,
      // Real delta (newly-enrolled balita this month). May be 0 — that's honest.
      perubahanBalitaBulanIni: newBalitaThisMonth,
      totalIbuHamil,
      // Module disabled in demo; delta is always 0.
      perubahanIbuHamilBulanIni: 0,
      persentaseGiziBaik,
      // Real delta (or null when no comparable historical data exists).
