...
import { Search, Baby, HeartPulse, Calculator, FileBarChart, LayoutDashboard, Syringe, Settings, ArrowRight, X, Droplet, Utensils, Calendar, BarChart3 } from 'lucide-react';
...
export function CommandPalette() {
  ...
  const navCommands: CommandItem[] = [
    ...
    { id: 'nav-imunisasi', label: 'Imunisasi', description: 'Catat & pantau imunisasi', icon: Syringe, action: () => setView('imunisasi'), group: 'Navigasi' },
    { id: 'nav-vitamin-a', label: 'Vitamin A', description: 'Distribusi kapsul Vitamin A', icon: Droplet, action: () => setView('vitamin-a'), group: 'Navigasi' },
    { id: 'nav-pmt', label: 'PMT', description: 'Pemberian Makanan Tambahan', icon: Utensils, action: () => setView('pmt'), group: 'Navigasi' },
    { id: 'nav-ibu-hamil', label: 'Data Ibu Hamil', description: 'Pemantauan ibu hamil', icon: HeartPulse, action: () => setView('ibu-hamil'), group: 'Navigasi' },
    { id: 'nav-jadwal', label: 'Jadwal Posyandu', description: 'Jadwal kegiatan mendatang', icon: Calendar, action: () => setView('jadwal'), group: 'Navigasi' },
    { id: 'nav-analitik', label: 'Analitik', description: 'Analisis data dan insight', icon: BarChart3, action: () => setView('analitik'), group: 'Navigasi' },
  ...
  const ibuCommands: CommandItem[] = ibuHamilList.map((i) => ({
    id: `ibu-${i.id}`,
    label: i.namaLengkap,
    description: `Usia hamil ${i.usiaKehamilanMinggu} minggu · ${i.status}`,
    icon: HeartPulse,
    action: () => setView('ibu-hamil'),
    group: 'Ibu Hamil',
  }));

  const allCommands = [...navCommands, ...balitaCommands, ...ibuCommands];
      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-xl border border-border bg-popover shadow-2xl">
        ...
        <div className="flex items-center gap-3 border-b border-border px-4 py-3">
          ...
          <input
            ...
            placeholder="Cari balita, ibu hamil, atau navigasi..."
